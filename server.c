...
int main(int argc, char *argv[])
{
    ...
    if (socket_desc == -1)
    {
        printf("Infelizmente não foi possível criar o socket\n");
        return 1;
    }
    ...
    {
        printf("Ocorreu um erro ao fazer o bind\n");
        return 1;
    }
    ...
    while ((new_socket = accept(socket_desc, (struct sockaddr *) &client, (socklen_t*) &c)))
    {
        ...
        do {
            ...
            /* resposta ao cliente */
            message = "Olá Cliente! Conexão recebida com sucesso!";
            write(new_socket, message, strlen(message));
        } while(strcmp(client_reply, "exit") != 0);
    }
    if (new_socket < 0) 
    {
        printf("Infelizmente sua conexão não foi aceita!\n");
        return 1;
    }
